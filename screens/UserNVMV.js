...
const UserNVMV = ({routes, navigation}) => {
  ...
  var VEHICLEFOUNDNOTIFICATION = firestore().collection('VEHICLEFOUNDNOTIFICATION');

  useEffect(() => {
    var Dataa = async () => {
      await VEHICLEFOUNDNOTIFICATION.get().then(data => {
...
const Chat = (item)=>{
    ...
    if (item.usercontact = "..."){
      ...
      if (item.usercontact != "..."){
        // Btnstate = false;
        let a =  item.usercontact
        let phoneNumber = "+92" + a// Replace with the phone number you want to send the message to
